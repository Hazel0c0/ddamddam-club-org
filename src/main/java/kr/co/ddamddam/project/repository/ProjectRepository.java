...
public interface ProjectRepository extends JpaRepository<Project, Long> {
    ...
    @Query("SELECT p FROM Project p " +
        ...
        "ORDER BY (p.maxBack - SIZE(p.applicantOfBacks)) ASC,(p.maxFront - SIZE(p.applicantOfFronts)) ASC, p.offerPeriod ASC,p.likeCount DESC ")
    Page<Project> backQuickSort(Pageable pageable);

    @Query("SELECT p FROM Project p " +
        "WHERE (p.maxFront - SIZE(p.applicantOfFronts)) > 0 " +
        "ORDER BY (p.maxFront - SIZE(p.applicantOfFronts)) ASC,(p.maxBack - SIZE(p.applicantOfBacks)) ASC, p.offerPeriod ASC,p.likeCount DESC ")
